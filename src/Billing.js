...
const handlePrint = () => {
  ...
  const toggleGroup = (taxRate) => {
    setOpenGroups(prev => {
      ...
      if (prev[taxRate] === undefined) {
        return { ...prev, [taxRate]: false }; 
      }
      return {
        ...prev,
        [taxRate]: !prev[taxRate]
  ...
  useEffect(() => {
  if (taxGroups.length > 0 && Object.keys(openGroups).length === 0) {
    const defaultOpenState = {};
    taxGroups.forEach(group => {
      defaultOpenState[group.taxRate] = true; 
    });
            ...
            <TableCell colSpan={7}>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                ...
                <IconButton
                  ...
                  onClick={() => toggleGroup(group.taxRate)}
                >
                  {openGroups[group.taxRate] !== false ? <KeyboardArrowUp /> : <KeyboardArrowDown />}
                </IconButton>
